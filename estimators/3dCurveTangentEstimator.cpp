...
int main(int argc, char **argv)
{
  ...
  {
    if (vm[ "connectivity" ].as<string>() == "26")
      ComputeLMSTBy2D  < PointIterator, Z3, std::vector< RealVector >, 8 > ( sequence.cbegin(), sequence.cend(), tangents, output );
    else
    {
      trace.info() << "L-MSTBy2D implemented only for 26-connected curves!" << endl;
      abort();
    }
